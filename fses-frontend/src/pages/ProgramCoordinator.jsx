...
import { BookOpen, Download, Lock, BarChart3, Search, CheckCircle, Clock, XCircle, UserPlus } from "lucide-react"
...
const AssignChairModal = ({
  ...
  if (!showModal || modalType !== "assign") return null

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Assign Chairperson - {editingStudent?.name}</h3>

        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Select Chairperson</label>
            <select
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <button onClick={onAssign} className="px-4 py-2 bg-burgundy-700 text-white rounded-md hover:bg-burgundy-800">
            Assign
          </button>
        </div>
      </div>
...
// StudentsList component defined outside to prevent re-renders
const StudentsList = ({
  searchTerm,
  setSearchTerm,
  filterStatus,
  ...
  handleLockNominations,
  lockedStatus,
  openModal,
  handleLockNomination,
  updateNomination,
}) => {

  const handleAutoAssign = async () => {
    const students = filteredStudents
      if (!students || !lecturers) {
        alert('No students or lecturers found!');
        return;
      }

      // Find all eligible chairpersons (professors and associate professors)
      const eligibleChairpersons = lecturers.filter((lect) => {
        return lect.title === 1 || lect.title === 2; // Must be Professor (1) or Associate Professor (2)
      });

      // Loop through each student and assign a chairperson if none exists
      let assignedCount = 0;

      for (const student of students) {
        // Skip if the student already has a chairperson
        const nomination = nominations.find((nom) => nom.student === student.id);

        if (!nomination) {
          console.log(`No nomination found for student ${student.name}`);
          continue; // Or create a new nomination here if required
        }

        if (nomination && nomination.chairperson) continue;

        // Find the eligible chairperson for this student
        const availableChair = eligibleChairpersons.find((chair) => {
          // Check if chairperson has been assigned 4 students in the same department
          const chairCount = nominations.filter(
            (nom) => nom.chairperson === chair.id && nom.student.department === student.department
          ).length;
          return chairCount < 4; // Must not exceed 4 students per department
        });

        if (availableChair) {
          // Assign the chairperson to this student
          await updateNomination(nomination.id, {
            ...nomination,
            chairperson: availableChair.id, // Assign chairperson to student
          });
      ...
      if (assignedCount > 0) {
        alert(`Successfully assigned ${assignedCount} chairpersons.`);
      } else {
        alert('No chairpersons were assigned. Please check eligibility and availability.');
      }
    };
      ...
      <div className="flex space-x-3">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg flex items-center space-x-2 ${
            lockedStatus ? "bg-gray-400 text-gray-700 cursor-not-allowed" : "bg-red-600 text-white hover:bg-red-700"
          }`}
        >
          <UserPlus size={16} />
          <span>Auto Assign Chairpersons</span>
        </button>
      </div>
    </div>
...
// StatisticsView component defined outside to prevent re-renders
const StatisticsView = ({ enrichedStudents, nominations }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-green-600">{stats.chairAssigned}</div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-yellow-600">{stats.pendingChair}</div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-red-600">{stats.pendingExaminers}</div>
            ...
            <div className="flex items-center">
              <div className="w-32 bg-gray-200 rounded-full h-2 mr-2">
                <div
                  ...
                  style={{
                    width: `${
                      (allStudents.filter((s) => s.evaluationType === "First Evaluation").length / allStudents.length) *
                      100
                    }%`,
                  }}
                ></div>
            ...
            <div className="flex items-center">
              <div className="w-32 bg-gray-200 rounded-full h-2 mr-2">
                <div
                  ...
                  style={{
                    width: `${
                      (allStudents.filter((s) => s.evaluationType === "Re-Evaluation").length / allStudents.length) *
                      100
                    }%`,
                  }}
                ></div>
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {currentPage === "students" && (
            <StudentsList
              ...
              handleLockNominations={handleLockNominations}
              lockedStatus={lockedStatus}
              openModal={openModal}
              handleLockNomination={handleLockNomination}
              updateNomination={updateNomination}
            />
          )}
          {currentPage === "statistics" && (
            <StatisticsView enrichedStudents={enrichedStudents} nominations={nominations} />
          )}
        </div>
      </div>
      ...
      <AssignChairModal
        ...
        onClose={closeModal}
        onAssign={handleAssignChairperson}
